...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^lineage_\|^rising_") ; then
        LINEAGE_BUILD=$(echo -n $1 | sed -e 's/^lineage_\|^rising_//g')
    else
        LINEAGE_BUILD=
    fi
...
function riseupload() {
    ...
    upload_version=$(echo "$(get_build_var RISING_VERSION)" | cut -d'.' -f1).x
    product_out="out/target/product/$target_device/"
    source_file="$product_out/${filename}.zip"
    destination="${sf_username}@frs.sourceforge.net:/home/frs/project/risingos-official/$upload_version/$package_type/$target_device/"
...
function opt_patch() {
    ...
    if [ -z "$TARGET_SKIP_ROM_OPTIMIZATION_PATCHING" ] || [ "$TARGET_SKIP_ROM_OPTIMIZATION_PATCHING" != "true" ]; then
        cat <<EOF
====================================================================
           Starting Automated ROM Optimization Patching
====================================================================
EOF
        ...
        cat <<EOF
====================================================================
                ROM Optimization Patches Applied
====================================================================
EOF
    else
        echo "Skipping automated patching (TARGET_SKIP_ROM_OPTIMIZATION_PATCHING was set to true)"
    fi
